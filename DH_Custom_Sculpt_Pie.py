...
class PIE_MT_SculptPie(Menu):
    bl_idname = "DH_MT_PIE_sculpt"
    bl_label = "DH Pie Sculpt"
    ...
    def draw(self, context):
        ...
        pie.operator("paint.brush_select",
                     text="    Draw Sharp", icon_value=brush_icons["draw"]).sculpt_tool = 'DRAW_SHARP'
        # 6 - RIGHT
        pie.operator("paint.brush_select",
                     text="    Scrape", icon_value=brush_icons["scrape"]).sculpt_tool = 'SCRAPE'
        # 2 - BOTTOM
        ...
        pie.menu(PIE_MT_Sculptthree.bl_idname,
                 text="    Transform Brushes", icon_value=brush_icons["grab"])
...
class PIE_MT_Sculpttwo(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("transform.shrink_fatten", text="Line Project").mode = 'LINE'
...
class PIE_MT_Sculptthree(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("wm.tool_set_by_id", text="Move Tool").name = "builtin.move"
        layout.operator("wm.tool_set_by_id", text="Rotate Tool").name = "builtin.rotate"
        layout.operator("wm.tool_set_by_id", text="Scale Tool").name = "builtin.scale"
        layout.operator("paint.brush_select",
